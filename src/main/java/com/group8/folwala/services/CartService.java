...
public class CartService {
    private static final String CART_FILE_PATH = System.getenv("APPDATA") + "\\Folwala\\carts\\";
    ...
    public static Cart loadCart(String userPhone) {
        ...
        if (file.exists()) {
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                ...
                System.out
                        .println("No cart found or failed to load cart for user " + userPhone + ": " + e.getMessage());
                e.printStackTrace();
            }
        }
        return loadedCart;
